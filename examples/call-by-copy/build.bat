...
if not %ERRORLEVEL%==0 (
    popd
    echo %_ERROR_LABEL% Filed to generate configuration files into directory "!_TARGET_DIR:%_ROOT_DIR%=!" 1>&2
    set _EXITCODE=1
    goto :eof
)
set __MAKE_OPTS=
...
if not %ERRORLEVEL%==0 (
    popd
    echo %_ERROR_LABEL% Faled to generate configuration files into directory "!_TARGET_DIR:%_ROOT_DIR%=!" 1>&2
    set _EXITCODE=1
    goto :eof
)
set __MAKE_OPTS=
