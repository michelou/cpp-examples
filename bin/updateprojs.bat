...
set _MAKE_DUMMY_VERSION_OLD=XXXX
set _MAKE_DUMMY_VERSION_NEW=XXXX
...
:run
for %%i in (concurrency-examples glfw-examples examples grimm-examples gui-examples pthreads-examples) do (
...
for /f %%i in ('dir /ad /b "%__PARENT_DIR%" ^| findstr /v /c:"lib"') do (
    ...
    if exist "!__MAKEFILE!" (
        if %_DEBUG%==1 echo %_DEBUG_LABEL% "%_GREP_CMD%" -q "%_COPYRIGHT_DATES_OLD%" "!__MAKEFILE!" 1>&2
        call "%_GREP_CMD%" -q "%_COPYRIGHT_DATES_OLD%" "!__MAKEFILE!"
        if !ERRORLEVEL!==0 (
            if %_DEBUG%==1 echo %_DEBUG_LABEL% "%_SED_CMD%" -i "s@%_COPYRIGHT_DATES_OLD%@%_COPYRIGHT_DATES_NEW%@g" "!__MAKEFILE!" 1>&2
            call "%_SED_CMD%" -i "s@%_COPYRIGHT_DATES_OLD%@%_COPYRIGHT_DATES_NEW%@g" "!__MAKEFILE!"
...
if exist "%__MAKEFILE_INC%" (
    if %_DEBUG%==1 echo %_DEBUG_LABEL% "%_GREP_CMD%" -q "%_MAKE_DUMMY_VERSION_OLD%" "!__MAKEFILE_INC!" 1>&2
    call "%_GREP_CMD%" -q "%_MAKE_DUMMY_VERSION_OLD%" "!__MAKEFILE_INC!"
    if !ERRORLEVEL!==0 (
        if %_DEBUG%==1 echo %_DEBUG_LABEL% "%_SED_CMD%" -i "s@%_MAKE_DUMMY_VERSION_OLD%@%_MAKE_DUMMY_VERSION_NEW%@g" "!__MAKEFILE_INC!" 1>&2
        call "%_SED_CMD%" -i "s@%_MAKE_DUMMY_VERSION_OLD%@%_MAKE_DUMMY_VERSION_NEW%@g" "!__MAKEFILE_INC!"
        call "%_UNIX2DOS_CMD%" -q "!__INPUT_FILE!"
        set /a __N_INC+=1
    )
) else (
